...
async function Init_UI() {
  ...
  $("#searchIcon").on("click", async function () {
    pageManager.update(false, true, false, false);
  });
...
async function renderPosts(queryString, isSearch, cantHaveNoResult) {
  let endOfData = false;
  ...
  if (isSearch) {
    ...
    } else queryString += lastQueryString;
  }
  ...
  if (!Posts_API.error) {
    ...
    if (posts.length > 0) {
      posts.forEach((post) => {
        ...
        $("#itemsPanel").append(
          renderPost(post, text, wordsLengthMore, "", "", showSeeMore, cantHaveNoResult)
        );
        ...
        if (wordsLengthMore) {
          $(".seeMoreDescriptionPost").on("click", function () {
            ...
            if (index !== -1) postsSeeMore.splice(index, 1);
      ...
      $("#itemsPanel").append(
        renderPost(null, "", false, filters["keywords"], filters["category"], false, cantHaveNoResult)
      );
    }
  } else {
...
function renderPost(
  ...
  if (post !== null) {
    ...
    if (wordsLengthMore) {
      if (!showSeeMore) {
        elementsWordsLengthMore = `<p class="descriptionPost text" completeDescription="${
          post.Id
        }" style="display: none">
                                        ...
                                        <p class="descriptionPost text" descriptionPostId="${
                                          post.Id
                                        }">
                                            ${textDescription}...
                                        </p>
                                        <p class="seeMoreDescriptionPost seeMore" seeMore="${
                                          post.Id
                                        }">Voir plus</p>
                                        <p class="seeLessDescriptionPost seeLess" seeLess="${
                                          post.Id
                                        }" style="display: none">Voir moins</p>
                                        `;
      } else {
        elementsWordsLengthMore = `<p class="descriptionPost text" completeDescription="${
          post.Id
                                            ...
                                            <p class="seeMoreDescriptionPost seeMore" seeMore="${
                                              post.Id
                                            }" style="display: none">Voir plus</p>
                                            <p class="seeLessDescriptionPost seeLess" seeLess="${
                                              post.Id
                                            }">Voir moins</p>
                                            `;
        ...
        <div class="postContainer" id="${post.Id}">
            <div class="globalTopInformationsContainer">
                ...
                <div class="iconsContainer">
                    <span class="cmdIcon fa fa-edit editCmd" title="Modifier la publication" editPostId="${
                      post.Id
                    }"></span>
                    <span class="cmdIcon fa fa-trash deleteCmd" title="Supprimer la publication" deletePostId="${
                      post.Id
                    }"></span>
                </div>
            </div>
            ...
            <div>
                <p class="dateLastModificationPost text">${convertToFrenchDate(
                  post.Creation
                )}</p>
            </div>
            <div>
    ...
    if (valueSearchBar !== "" && !cantHaveNoResult) {
...
function renderPostForm(post = null) {
  ...
  $("#thePostForm").on("submit", async function (event) {
    ...
    if (!Posts_API.error) {
      showPosts();
      setAllCategories();
      if (selectedCategory == "" || post.Category == oldCategory){
        await pageManager.update(false, false, false, false);
        pageManager.scrollToElem(create ? result.Id : post.Id); //To go at the element in the scroll view
      }
      else{
        await pageManager.update(false, true, false, false);
      }
    } else renderError("Une erreur est survenue! ");
    removeWaitingGif();
  });
...
async function updateSelectOfCategories(categories) {
  ...
  $("#searchSelect").on("change", function () {
    ...
    selectedCategory = "";
    if (val !== "all") selectedCategory = val;
  });
...
function filterKeywords() {
  ...
  if (keywordsTab.length == 0) {
    //searchValue was empty
    if ("keywords" in filters) {
      //Check if "keywords" key was in the filters object
